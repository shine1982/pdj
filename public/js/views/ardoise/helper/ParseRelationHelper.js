    ...
    if(item){//trouvé
        ...
        query.find().then(function(results){
            if(results.length>0){
                ...
                self.updateRelationFromList(objParent,nameOfRelation,collectionModel);
...
app.parseRelationHelper.updateRelationFromList=function(objParent, relationName, collectionModel){
...
app.parseRelationHelper.deleteItemFromRelation=function(objParent, relationName, collectionModel, item){
    var relation = objParent.relation(relationName);
    relation.query().find({
        success:function(results){
            if(results!=null && results.length>0){
